...
namespace eye
{
	...
	void ColorDetector::imageCallback(const sensor_msgs::ImageConstPtr &image_rect)
	{
		...
		cv::GaussianBlur(srcImg, srcGauss, cv::Size(7, 7), 0, 0);
		cv::cvtColor(srcGauss, srcHSV, cv::COLOR_BGR2HSV);
		cv::Mat element = cv::getStructuringElement(cv::MORPH_ELLIPSE, Size(25, 25));
		cv::inRange(srcHSV, Low[RGB], Up[RGB], srcImg);
		cv::erode(srcImg, srcImg, element);

		std::vector<std::vector<cv::Point>> contours;									  //轮廓容器
		cv::Point2f vtx[4];													  //矩形顶点容器
		cv::findContours(srcImg, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE); //查找轮廓
		cv::Mat dst = cv::Mat::zeros(srcImg.size(), CV_8UC3);					  //创建空白图像
