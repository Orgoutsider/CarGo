...
返回  值：无
**************************************************************************/
void judge_Callback(const motion_controller::statusConstPtr &status_p)
{
    if(flag_cor)return;
    last_err = stts.err;
    stts = *status_p;
...
int main(int argc, char **argv)
{
    ...
    /***创建弯道检测话题订阅者***/
    ros::Subscriber judge_sub = node.subscribe<motion_controller::status>("corners_judge", 1, judge_Callback);
