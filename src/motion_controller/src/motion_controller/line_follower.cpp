...
namespace motion_controller
{
    LineFollower::LineFollower(ros::NodeHandle &nh, ros::NodeHandle &pnh)
        : r_start_(105), r_end_(180),
          c_start_(5), c_end_(width_ - c_start_),
          threshold_(160), Hough_threshold_(50),
        ...
        if (lines.size() > 1)
        {
            ...
            for (Vec2f &line : lines)
            {
                ...
                if (param_modification_)
                {
                    ...
                    Mat HoughLines(srcF.size(), CV_8UC3, Scalar::all(0));
                    ...
                    cv::line(HoughLines, pt1, pt2, Scalar(0, 0, 255), 1, LINE_AA);
