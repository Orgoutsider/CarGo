...
{
    VisionFollower::VisionFollower(ros::NodeHandle &nh, ros::NodeHandle &pnh)
          ...
          startup_(false), start_image_sub_(false)
    {
        ...
        if (debug_)
        {
            ...
            dr_server_.setCallback(boost::bind(&VisionFollower::_dr_callback, this, _1, _2));
        }
    }

    void VisionFollower::_clean_lines(cv::Vec2f lines[], int num, double &rho_aver, double &theta_aver)
    {
        if (num == 0)
        {
    ...
    // bool VisionFollower::_color_judge(cv::Vec2f &line, bool left)
    // {
    //     double rho = line[0];
    //     double theta = line[1];
    ...
    bool VisionFollower::_find_lines(cv_bridge::CvImagePtr &cv_image, geometry_msgs::Twist &twist)
    {
        using namespace cv;
        // 色彩分离查找车道线
        Mat srcF;
    ...
    // bool VisionFollower::_find_road(cv_bridge::CvImagePtr &cv_image, geometry_msgs::Twist &twist)
    // {
    //     using namespace cv;
    ...
    void VisionFollower::_image_callback(const sensor_msgs::ImageConstPtr &image_rect)
    {
        static int cnt = 0;
        if (cnt < 10) // 等待摄像头适应光线
        {
    ...
    void VisionFollower::_dr_callback(lineConfig &config, uint32_t level)
    {
        ...
        if (theta_thr_horizontal_ != config.theta_thr_horizontal)
            theta_thr_horizontal_ = config.theta_thr_horizontal;
        if (kp_ != config.kp)
            kp_ = config.kp;
        if (kd_ != config.kd)
            kd_ = config.kd;
    ...
    bool VisionFollower::_do_start_req(Start::Request &req, Start::Response &resp)
    {
        if (req.start)
        {
            if (!start_image_sub_)
    ...
    void VisionFollower::plot_line(cv::Mat &mat, double rho, double theta, cv::Scalar color, double width)
    {
        cv::Point pt1, pt2;
