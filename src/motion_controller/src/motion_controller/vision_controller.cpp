#include "motion_controller/vision_controller.h"

namespace motion_controller
{
    VisionController::VisionController(ros::NodeHandle &nh, ros::NodeHandle &pnh)
        : cnt_(0),
          ...
          distance_thr_(0.05), startup_(false),
          ...
          ac_move_(nh, "Move", true), ac_arm_(nh, "Arm", true)
    {
        ...
        if (debug_)
        {
            dr_server_.setCallback(boost::bind(&VisionController::_dr_callback, this, _1, _2));
            start_image_subscriber(true);
        }
        vision_publisher = nh.advertise<Distance>("/vision_usb_cam", 5);
        timer_ = nh.createTimer(ros::Rate(3), &VisionController::_timer_callback, this, false, false);
        go_client_ = nh.advertiseService("Go", &VisionController::go, this);
        start_client_ = nh.serviceClient<Start>("Start");
    }

    bool VisionController::_turn()
    {
    ...
    bool VisionController::_turn_by_position()
    {
        if (!ac_move_.isServerConnected())
            ac_move_.waitForServer();
        if (get_position())
    ...
    void VisionController::_clean_lines(double y[], double &y_sum, int &tot)
    {
        if (tot <= 0)
        {
    ...
    bool VisionController::_color_judge(cv::Mat &img, bool note[], cv::Vec2f &line)
    {
        double rho = line[0];
        double theta = line[1];
        double a = cos(theta);
    ...
    void VisionController::_image_callback(const sensor_msgs::ImageConstPtr &image_rect)
    {
        static int count = 0;
        ...
        if (lines.size())
        {
            ...
            else if (cnt_ >= cnt_tolerance_ && !debug_ && tot == 0)
            {
                start_vision_follower(true);
                cnt_ = 0;
                if (!timer_.hasStarted())
    ...
    void VisionController::_timer_callback(const ros::TimerEvent &event)
    {
        if (get_position())
        {
            // 转弯结束关闭转弯订阅
            ...
            if (arrive())
            {
                ...
                ac_arm_.sendGoal(goal, boost::bind(&VisionController::_done_callback, this, _1, _2),
                                 boost::bind(&VisionController::_active_callback, this),
                                 boost::bind(&VisionController::_feedback_callback, this));
                doing();
                start_vision_follower(false);
            }
        }
    ...
    void VisionController::_dr_callback(motion_controller::cornersConfig &config, uint32_t level)
    {
        if (!debug_)
            return;
        // return;
    ...
    void VisionController::_active_callback()
    {
        if (timer_.hasStarted())
            timer_.stop();
    ...
    void VisionController::_feedback_callback() {}

    void VisionController::_done_callback(const actionlib::SimpleClientGoalState &state, const my_hand_eye::ArmResultConstPtr &result)
    {
    ...
    bool VisionController::set_position(double x, double y, double theta)
    {
        x_ = x;
        y_ = y;
        // 将theta_限定在(-pi,pi]之间
    ...
    bool VisionController::get_position()
    {
        try
        {
            //   解析 base_footprint 中的点相对于 odom_combined 的坐标
    ...
    bool VisionController::start_vision_follower(bool start)
    {
        ...
        {
            if (start)
                ROS_INFO("start VisionFollower.");
            else
                ROS_INFO("shut down VisionFollower.");
        }
        else
            ROS_WARN("Invalid request! VisionFollower cannot start/shutdown.");
        return flag;
    }

    void VisionController::start_image_subscriber(bool start)
    {
        if (start)
        {
            cnt_ = 0;
            image_subscriber_ = it_->subscribe("/usb_cam/image_rect_color", 1,
                                               &VisionController::_image_callback, this,
    ...
    bool VisionController::go(Go::Request &req, Go::Response &resp)
    {
        if (!ac_move_.isServerConnected())
            ac_move_.waitForServer();
    ...
    void VisionController::plot_line(cv::Mat &mat, double rho, double theta, cv::Scalar color)
    {
        cv::Point pt1, pt2;
        double a = cos(theta), b = sin(theta);
