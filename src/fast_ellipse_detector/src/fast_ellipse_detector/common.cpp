...
#include "fast_ellipse_detector/common.h"
...
#define c_P15 imgIT(y + 2, x + 2) == 1
				...
				int apertureSize)
	{
		...
		// aperture_size = -1; //SCHARR
		Sobel(src, c_dx, CV_16SC1, 1, 0, apertureSize);
		Sobel(src, c_dy, CV_16SC1, 0, 1, apertureSize);

		double maxVal;
		Mat1f magnitude;
		c_dx.convertTo(c_dx, CV_32FC1);
		c_dy.convertTo(c_dy, CV_32FC1);
		cv::magnitude(c_dx, c_dy, magnitude);
		c_dx.convertTo(c_dx, CV_16SC1);
		c_dy.convertTo(c_dy, CV_16SC1);
		...
		// 自动设置canny阈值
		{
			...
			minMaxLoc(magnitude, &minVal, &maxVal, &minLoc, &maxLoc);
			//% Normalize for threshold selection
		...
		}
		{
			...
			{
				float *_pmag = magnitude.ptr<float>(i);
