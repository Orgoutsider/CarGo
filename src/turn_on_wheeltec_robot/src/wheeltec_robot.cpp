...
bool turn_on_robot::Get_Sensor_Data_New()
{
  ...
  */
  ...
  if (Receive_Data_Pr[0] == FRAME_HEADER || count > 0) // Ensure that the first data in the array is FRAME_HEADER //确保数组第一个数据为FRAME_HEADER
    ...
    count = 0;
  if (count == 12) // Verify the length of the packet //验证数据包的长度
  {
    ...
    {
      check = Check_Sum(10, READ_DATA_CHECK); // BCC check passes or two packets are interlaced //BCC校验通过或者两组数据包交错

      if (check == Receive_Data.rx[10])
      {
...
turn_on_robot::turn_on_robot() : Sampling_Time(0), Power_voltage(0)
{
  ...
  private_nh.param<std::string>("usart_port_name",  usart_port_name,  "/dev/c_board"); //Fixed serial port number //固定串口号
